...
export default function SkincareList() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true);
        const data = await skincareAPI.getAll();
        setSkincare(data);
      } catch (error) {
  ...
  const filteredSkincare = skincare.filter((item) =>
    item.name.toLowerCase().includes(search.toLowerCase())
  );

  const totalPages = Math.ceil(filteredSkincare.length / PER_PAGE);
  const displayedSkincare = filteredSkincare.slice(
    (page - 1) * PER_PAGE,
    page * PER_PAGE
  );
  ...
  return (
    <section className="px-6 py-12 max-w-7xl mx-auto">
      ...
      {/* Pagination */}
      {filteredSkincare.length > PER_PAGE && (
